...
const SetsCard: React.FC<SetsCardProps> = ({ title, sets }) => {
  ...
  return (
    <div className="bg-[#5a776b]/20  border border-primary/30 cursor-pointer p-4 rounded-lg shadow-lg" onClick={()=>setIsOpen(!isOpen)}>
      <h2 className="text-xl font-medium text-white flex items-center justify-between">
      ⁍ {title} <ChevronDownIcon className={`duration-200 ${isOpen ?  "rotate-180" : "rotate-0"}`}/>
      </h2>
      {isOpen && (
        ...
        {sets.map((set, index) => (
          <li
            key={index}
            className="flex justify-between items-center bg-white/10 border border-primary/30 p-2 rounded-md"
          >
            <span className="text-gray-300 ml-2">⁍ {set.name}</span>
            <a
              href={set.path}
