...
const Carousel: React.FC = () => {
  ...
  const items = slidesData.map((slide) => (
    <div key={slide.id} className="bg-[#55555562] p-8 sm:rounded-xl shadow-lg flex items-center justify-between h-[12vh] sm:h-[10vh]">
      <h2 className="text-white md:text-start text-center 2xl:text-4xl text-lg md:text-2xl font-semibold">
        {slide.title}
      </h2>
      <span className="text-5xl font-black text-[#0ac895] mx-4">&rarr;</span>
      <p className="text-[#0ac895] md:text-start text-center font-semibold 2xl:text-4xl text-lg md:text-2xl">
        {slide.description}
      </p>
    </div>
  ));
