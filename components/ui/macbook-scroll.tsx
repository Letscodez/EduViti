...
export const MacbookScroll = ({
  ...
  const scaleX = useTransform(
    scrollYProgress,
    [0, 0.3],
    [1.2, isMobile ? 1.25 : 1.75],
  );
  const scaleY = useTransform(
    scrollYProgress,
    [0, 0.3],
    [0.6, isMobile ? 1.25 : 1.75 ],
  );
  const translate = useTransform(scrollYProgress, [0, 1], [0, 1200]);
  const rotate = useTransform(scrollYProgress, [0.1, 0.12, 0.3], [-28, -28, 0]);
  const textTransform = useTransform(scrollYProgress, [0, 0.3], [0, 1]);
  ...
  return (
    ...
    >
      <motion.h2
        ...
        className="text-white/90 text-6xl font-bold mb-20 text-center"
      >
        <span className="text-primary">Why</span> Eduviti?
      </motion.h2>
      {/* Lid */}
      <Lid
        src={mac}
